...
mySemaphore::mySemaphore(string& sName,long initialCount,long maximumCount,int accessMode,BOOL canInherit)
{
	...
	if ( accessMode == CREATE )
    {
        ...
        if ( (semaphore = CreateSemaphore(sa,initialCount,maximumCount,(LPCWSTR)sName.c_str())) == NULL)
        {
			int errCode = 0;
    ...
    {
        if ((semaphore = OpenSemaphore(SEMAPHORE_ALL_ACCESS,canInherit,(LPCWSTR)sName.c_str())) == NULL)
        {
			int errCode;
...
void mySemaphore::getErrorDetail(int& errCode,string& header)
{
	...
	wsprintf(buf,L": %s",(LPTSTR)lpDetailMsgBuf);
    LocalFree(lpDetailMsgBuf);	
	
	header.append(string((char*)buf));
	errCode = errorCode;
}
